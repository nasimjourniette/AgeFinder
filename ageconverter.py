...
def check_for_leap_year():
    ...
    for year in range(date_year):
        amount_of_days = []
        # if the year is divisible by 4 except if it's divisible by 100 and not 400 then it's a leap year and february's days get changed to 29
...
def separate_birthdate(birthdate):
    ...
    for numbered_month in months:
        if numbered_month['month'] == int(month):
            if int(year) % 4 == 0 and not (int(year) % 100 == 0 and int(year) % 400 != 0) and numbered_month['month'] == 2 and int(day) == 29:
...
def find_age(birthday):
    ...
    # iterate from the birth year to the current year to find out if the birth year is a leap year and the nod in each year
    ...
    if int(m) == date_month and int(d) == date_day:
        return (
            f"You are {round(sum(nod) * 86400 + int(hour) * 3600 + int(minute) * 60 + float(second), 5)} seconds old.\n"
        ...
        if int(y) % 4 == 0 and amount_days > 60 or int(y) % 4 != 0:
            ...
            return (
                f"You are {round(nod_alive * 86400 + int(hour) * 3600 + int(minute) * 60 + float(second), 5)} seconds old.\n"
